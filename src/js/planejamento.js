...
    new Autocomplete(destinationInput);
    ...
    // O 'click' agora chama a função 'calculateAndDisplayRoute' que corrigimos
...
// *** INÍCIO DA CORREÇÃO 1: O código abaixo estava "solto" e foi colocado dentro da função ***
...
    };
    ...
    // *** CORREÇÃO 3 (MELHORIA): Adicionado 'async' aqui para usar 'await' dentro ***
...
            directionsRenderer.setDirections(result); 
            ...
            // *** CORREÇÃO 3 (MELHORIA): Adicionado 'await' para esperar as estações carregarem ***
            await findChargingStations(result);

            // *** MELHORIA ADICIONAL: Chamando sua função de cálculo avançado ***
            // Verifica se o usuário está logado para mostrar os dados avançados
            if (usuarioEstaLogado) {
                const distanciaEmKm = rota.distance.value / 1000;
                calcularDadosAvancados(distanciaEmKm); // Chama a função
                if(advancedDetails) advancedDetails.style.display = 'block'; // Mostra os detalhes
            } else {
                 if (loginPrompt) loginPrompt.style.display = 'block'; // Pede login
            }
